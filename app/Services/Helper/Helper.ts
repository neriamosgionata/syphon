...
import SemrushDatabasesEnum from "App/Enums/SemrushDatabasesEnum";
import CountriesEnum from "App/Enums/CountriesEnum";
import ManagedCountriesEnum from "App/Enums/ManagedCountriesEnum";
...
export interface HelperContract {
	...
	toSemrushDatabase(country: string): any;

	isoCountry(name: string | null | undefined): {
		iso2: string,
		iso3: string,
		name: string,
		localName: string,
	...
	isoCountryManaged(name: string | null | undefined): {
		iso2: string,
		iso3: string,
		name: string,
		localName: string,
	...
	computeCentroid(markers: IMarker[]): ILocation;
...
export default class Helper implements HelperContract {
	...
	toSemrushDatabase(country: string): any {
		...
		// Countries not supported by SemRush fallback to 'us'
		// Mismatching countries (of the ones currently in restaurant table):
		//
		//	MAC --> us // missing from semrush
		//	CHN --> us // missing from semrush
		//	USA --> us // missing from semrush
		//	GBR --> us // mislabeled as 'uk' instead of 'gb'
		//	AND --> us // missing from semrush
		//	SMR --> us // missing from semrush
		//	TWN --> us // missing from semrush
		//

		let database = "uk";

		const countryComputed = isoCountry(country) || this.isoCountry(country);

		if (countryComputed) {
			const found = SemrushDatabasesEnum.find(db => db.code === countryComputed.iso2.toLowerCase());

			if (!found) {
				database = 'us';
			} else {
				database = found.code === 'gb' ? 'uk' : found.code;
			}
		}
	...
	isoCountry(name: string | null | undefined): {
		...
		return CountriesEnum.find((country) => {
			return country.iso2 === name.toUpperCase() ||
				country.iso3 === name.toUpperCase() ||
				this.levenshteinDistance(country.name.toLowerCase(), name.toLowerCase()) < (name.length * 0.15) ||
				this.levenshteinDistance(country.localName.toLowerCase(), name.toLowerCase()) < (name.length * 0.15);
		});
	}
	...
	isoCountryManaged(name: string | null | undefined): {
		...
		return ManagedCountriesEnum.find((country) => {
			return country.iso2 === name.toUpperCase() ||
				country.iso3 === name.toUpperCase() ||
				this.levenshteinDistance(country.name.toLowerCase(), name.toLowerCase()) < (name.length * 0.15) ||
				this.levenshteinDistance(country.localName.toLowerCase(), name.toLowerCase()) < (name.length * 0.15);
	...
	computeCentroid(markers: IMarker[]): ILocation {
		...
		for (const marker of markers) {
			const lat = this.degr2rad(marker.lat);
			const lng = this.degr2rad(marker.lng);
	...
	distanceBetweenPoints(
		...
		//this is a calculation in spherical coordinates
		const R = 6371;
