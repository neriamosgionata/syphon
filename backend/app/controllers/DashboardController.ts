...
export default class DashboardController {
  ...
  public async activeJobs({ response }: HttpContext) {
    const jobs = await QueueService.getActiveJobs()
    return response.json({ jobs })
  }

  public async failedJobs({ request, response }: HttpContext) {
    const limit = request.input('limit', 50)
    const jobs = await QueueService.getFailedJobs(limit)
    return response.json({ jobs })
  }

  public async retryJob({ params, response }: HttpContext) {
  ...
  public async removeFailedJob({ params, response }: HttpContext) {
    const { queue, id } = params
  ...
  public async cancelJob({ params, response }: HttpContext) {
    const { queue, id } = params
  ...
  public async drainQueues({ response }: HttpContext) {
  ...
  public async prune({ response }: HttpContext) {
    logger.info('Pruning entire database...')

    // Delete from MariaDB (only small tables remain)
    await db.rawQuery('DELETE FROM trades')
    await db.rawQuery('DELETE FROM scrape_sources')
    ...
    logger.info('db pruned successfully')
