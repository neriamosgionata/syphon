...
class KrakenService {
  ...
  public async connect(): Promise<boolean> {
    ...
    try {
      ...
      logger.error('[Kraken] Connection failed: %s', (error as Error).message)
  ...
  public async placeOrder(trade: Trade): Promise<Trade> {
    try {
      ...
      await trade.save()
      logger.error('[Kraken] Failed to place order: %s', (error as Error).message)
      return trade
    }
  }
  ...
  public async cancelOrder(trade: Trade): Promise<Trade> {
    try {
      ...
      trade.errorMessage = (error as Error).message
      await trade.save()
      logger.error('[Kraken] Failed to cancel order: %s', (error as Error).message)
      return trade
    }
  }
  ...
  public async syncOrderStatus(trade: Trade): Promise<Trade> {
    ...
    try {
      ...
      logger.error('[Kraken] Failed to sync order %s: %s', txid, (error as Error).message)
      return trade
    }
  }
  ...
  public async getFuturesAccounts(): Promise<any> {
    try {
      ...
      logger.error('[Kraken Futures] Failed to get accounts: %s', (error as Error).message)
      return {}
    }
  }
  ...
  public async getFuturesPositions(): Promise<any[]> {
    try {
      ...
      logger.error('[Kraken Futures] Failed to get positions: %s', (error as Error).message)
      return []
    }
  }
  ...
  public async getFuturesOpenOrders(): Promise<any[]> {
    try {
      ...
      logger.error('[Kraken Futures] Failed to get open orders: %s', (error as Error).message)
      return []
    }
  }
  ...
  public async placeFuturesOrder(trade: Trade): Promise<Trade> {
    try {
      ...
      trade.errorMessage = (error as Error).message
      await trade.save()
      logger.error('[Kraken Futures] Failed to place order: %s', (error as Error).message)
      return trade
    }
  }
  ...
  public async syncFuturesOrderStatus(trade: Trade): Promise<Trade> {
    ...
    try {
      ...
      logger.error('[Kraken Futures] Failed to sync order %s: %s', orderId, (error as Error).message)
      return trade
    }
  }
