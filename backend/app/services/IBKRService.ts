...
import type { TradeSide, OrderType } from '#models/Trade'
...
/**
 * @stoqey/ib 1.6.7 only ships `on()` typings for a handful of events,
 * although the runtime emits every EventName. Reopen `on`/`once`/`off`
 * against the underlying EventEmitter so order/position/account events type-check.
 */
type IbConnection = IBApi & {
  on: (event: string, listener: (...args: any[]) => void) => IBApi
  once: (event: string, listener: (...args: any[]) => void) => IBApi
  off: (event: string, listener: (...args: any[]) => void) => IBApi
}

class IBKRService {
  private ib: IbConnection | null = null
  ...
  public async connect(): Promise<boolean> {
    ...
    return new Promise((resolve) => {
      try {
        this.ib = new IBApi({ host, port, clientId }) as IbConnection
        ...
        logger.error('[IBKR] Connection error: %s', (error as Error).message)
        resolve(false)
      }
    })
  }
  ...
  private setupEventHandlers() {
    ...
    this.ib.on(EventName.error, (err: Error, code: number, reqId: number) => {
      ...
      if ([2104, 2106, 2158].includes(code)) {
        logger.debug('[IBKR] Info %d: %s', code, (err as Error).message)
        return
      }
      logger.error('[IBKR] Error %d (reqId=%d): %s', code, reqId, (err as Error).message)
    })
  ...
  public async placeOrder(trade: Trade): Promise<Trade> {
    ...
    try {
      ...
      await trade.save()
      logger.error('[IBKR] Failed to place order: %s', (error as Error).message)
    }
  ...
  public async cancelOrder(trade: Trade): Promise<Trade> {
    ...
    try {
      ...
      logger.error('[IBKR] Failed to cancel order %d: %s', trade.ibOrderId, (error as Error).message)
      trade.errorMessage = (error as Error).message
      await trade.save()
    }
