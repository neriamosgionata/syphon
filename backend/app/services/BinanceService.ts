...
class BinanceService {
  ...
  public async connect(): Promise<boolean> {
    ...
    try {
      ...
      logger.error('[Binance] Connection failed: %s', (error as Error).message)
  ...
  public async placeOrder(trade: Trade): Promise<Trade> {
    try {
      ...
      await trade.save()
      logger.error('[Binance] Failed to place order: %s', (error as Error).message)
      return trade
    }
  }
  ...
  public async cancelOrder(trade: Trade): Promise<Trade> {
    try {
      ...
      trade.errorMessage = (error as Error).message
      await trade.save()
      logger.error('[Binance] Failed to cancel order: %s', (error as Error).message)
      return trade
    }
  }
    ...
    try {
      ...
      if (volExec > 0) {
        try {
          ...
          logger.debug('[Binance] Commission sync failed for order %s: %s', orderId, (err as Error).message)
        }
      }
      ...
      logger.error('[Binance] Failed to sync order %s: %s', orderId, (error as Error).message)
      return trade
    }
  }
