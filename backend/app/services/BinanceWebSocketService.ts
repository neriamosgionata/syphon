...
class BinanceWebSocketService {
  ...
  public async connectUserData(): Promise<void> {
    ...
    try {
      ...
      logger.error('[BinanceWS] Failed to get listen key: %s', (err as Error).message)
    }
  }
  ...
  private connectStream(name: StreamName, url: string): void {
    ...
    if (env.get('NODE_ENV') === 'test') {
      logger.debug('[BinanceWS] Skipping %s stream connection in test env', name)
    ...
    ws.on('error', (err: Error) => {
      logger.error('[BinanceWS] %s stream error: %s', name, (err as Error).message)
    })
  ...
  private async refreshListenKey(): Promise<void> {
    ...
    try {
      ...
      logger.warn('[BinanceWS] Listen key refresh failed: %s', (err as Error).message)
    }
  }
