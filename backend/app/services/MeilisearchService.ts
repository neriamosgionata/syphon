...
class MeilisearchService {
  ...
  constructor(private redisClient: { incr: (key: string) => Promise<number> } = redis) {
  ...
  private async ensurePrimaryKey(uid: string, primaryKey: string) {
    ...
    try {
      ...
      logger.warn('[Meili] primaryKey set failed for %s: %s', uid, (err as Error).message)
    }
  }
  ...
  public async nextId(key: string): Promise<number> {
    return this.redisClient.incr(`meili:id:${key}`)
  }
  ...
  async deleteDocs(index: string, ids: (string | number)[]) {
