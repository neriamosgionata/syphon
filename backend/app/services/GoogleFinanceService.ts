...
class GoogleFinanceService {
  ...
  private async fetchHTML(url: string, retries = 3): Promise<string> {
    for (let attempt = 1; attempt <= retries; attempt++) {
      ...
      try {
        ...
        if (attempt < retries && !(error as Error).message.includes('after all retries')) {
  ...
  public async fetchQuote(symbol: string, knownExchange?: string) {
    try {
      ...
      logger.error('[GoogleFinance] Failed to fetch quote for %s: %s', symbol, (error as Error).message)
      return null
    }
  }
  ...
  private async fetchHistoricalFromStooq(
    ...
    try {
      ...
      logger.warn('[Stooq] Failed for %s: %s', symbol, (error as Error).message)
      return []
    }
  }
  ...
  private async fetchHistoricalFromGooglePage(
    ...
    try {
      ...
      logger.error('[GoogleFinance] Failed to fetch historical for %s: %s', symbol, (error as Error).message)
      return []
    }
  }
  ...
  public async searchTicker(query: string) {
    try {
      ...
      logger.error('[GoogleFinance] Failed to search ticker: %s', (error as Error).message)
      return []
    }
  }
  ...
  public async syncTickersBulk(symbols: string[], syncHistory = false): Promise<number> {
    ...
    for (let i = 0; i < symbols.length; i++) {
      ...
      try {
        ...
        logger.error('[GoogleFinance:BulkSync] Failed to sync %s: %s', symbol, (error as Error).message)
      }
    }
