...
class QueueService {
  ...
  public registerWorker(
    ...
    worker.on('failed', (job, err) => {
      logger.error('[%s] Job %s failed: %s', name, job?.id, (err as Error).message)
      if (job) {
        NotificationService.emit({
          type: 'job_finished',
          queue: name,
    ...
    worker.on('progress', (job, progress) => {
      const p = (typeof progress === 'object' ? progress : { progress, stage: '' }) as any
      NotificationService.emit({
        type: 'job_progress',
        queue: name,
        jobId: job.id!,
  ...
  public async getActiveJobs() {
    ...
    for (const name of Object.values(QUEUE_NAMES)) {
      ...
      for (const job of [...active, ...waiting, ...delayed]) {
        ...
        const p = (typeof progress === 'object' ? progress : { percent: progress || 0, stage: '' }) as any
        jobs.push({
          id: job.id,
          queue: name,
          state,
